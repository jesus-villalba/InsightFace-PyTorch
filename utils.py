...
def draw_bboxes(img, bounding_boxes, facial_landmarks=[]):
    for b in bounding_boxes:
        cv.rectangle(
            img, (int(b[0]), int(b[1])), (int(b[2]), int(b[3])), (255, 255, 255), 1
        )
...
def parse_args():
    ...
    # general
    parser.add_argument(
        "--pretrained", type=bool, default=False, help="pretrained model"
    )
    parser.add_argument("--network", default="r101", help="specify network")
    parser.add_argument(
        "--end-epoch", type=int, default=1000, help="training epoch size."
    )
    parser.add_argument("--lr", type=float, default=0.1, help="start learning rate")
    parser.add_argument(
        "--lr-step", type=int, default=10, help="period of learning rate decay"
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, default=256, help="batch size in each context"
    )
    ...
    parser.add_argument(
        "--gamma", type=float, default=2.0, help="focusing parameter gamma"
    )
